...
# You need to implement this method as a method of the LinkedList class. The method should take an integer
# x as input. If the linked list is empty, the method should return None.

# To implement this method, you should create two new linked lists. These two linked lists 
# will be made up of the original nodes from the linked list that is being partitioned, one 
# for nodes less than x and one for nodes greater than or equal to x.  
...
# The partition_list method separates the nodes into two lists, one for nodes with values less than 
# x and the other for nodes with values equal to or greater than x. The function then concatenates these two lists.

# In the above example, the nodes with values less than 4 (i.e., 2, 1, and 3) come before the nodes 
# with values equal to or greater than 4 (i.e., 8, 4, and 7).  The relative order of the nodes is preserved.
